...
static int setaffinity()
{
    ...
    err = sched_setaffinity(getpid(), sizeof(cpu_set) / sizeof(cpu_set[0]), cpu_set);
    if (err < 0) {
        cerr << "sched_setaffinity: " << strerror(errno) << endl;
        return -1;
    }
...
static int get_cpu_stat(CpuStat &cpu_stat)
{
    string line;
    stringstream ss;
    uint64_t user_hz;
...
static int get_cpu_percent(const CpuStat &last, CpuStat &now, int &percent)
{
    get_cpu_stat(now);
