...
#define USER_SIG2 SIGRTMAX
#define USER_SIG1 (SIGRTMAX - 1)
...
static void handler1(int num, siginfo_t *info, void *uc)
{
	...
	sigset_t set;
	fprintf(stderr, "%d [%d] sig received, now I am waiting\n", num, tid);
...
//	sleep(1);
	while(!lock){};
	fprintf(stderr, "%d [%d] I have passed the barrier\n", num, tid);
}

static void handler2(int num, siginfo_t *info, void *uc)
{
	pid_t tid = syscall(SYS_gettid);
	fprintf(stderr, "%d [%d] make handler1 go ahead\n", num, tid);
	lock = 1;
}
